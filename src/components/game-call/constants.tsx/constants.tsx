...
export const createRandomNumberArray = (count: number, [min, max]: number[]): Array<number> => {
  ...
  if (count > (max - min + 1)) throw new Error('We can\'t do this Array. Do count more');

  return Array(count).fill(0).map((_) => uniqNumber(min, max));
};
